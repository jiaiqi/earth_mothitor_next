...
function openNewsDetail(row: any) {
  ...
  else if (props.type === '最新地震动态') {
    navigateTo({
      path: `/earthquakeInfo/situation/${row.id}`,
      query: {
        data: encode(row.item),
      },
    })
  }
  else if (props.type === '历史上的今天') {
    navigateTo({
...
<template>
  <ul class="">
    <li
      v-for="item in props.newsList" :key="item.title" flex="~ items-center justify-between "
    ...
    >
      ...
      <el-tooltip
        v-if="showDate !== false && item.date" class="box-item" effect="dark"
